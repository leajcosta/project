...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True, transforms=None):
        ...
        n = len(sources)
        self.imgs, self.depths,self.fps, self.frames, self.threads = [None] * n,[None] * n, [0] * n, [0] * n, [None] * n
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.imgs[i] = np.asanyarray(color_frame.get_data())  # guarantee first frame
            self.depths[i] = aligned_depth_frame #np.asanyarray(aligned_depth_frame.get_data())

            self.threads[i] = Thread(target=self.update, args=([i, pipeline,align, s]), daemon=True)
    ...
    def update(self, i, pipeline, align, stream):
        ...
        while n < f:
            ...
            if n % read == 0:
                frame_ = pipeline.wait_for_frames()
                if frame_:
                    aligned_frame_ = align.process(frame_)

                    # Get aligned frames
                    aligned_depth_frame = aligned_frame_.get_depth_frame()  # aligned_depth_frame is a 640x480 depth image
                    color_frame = aligned_frame_.get_color_frame()
    ...
    def __next__(self):
        ...
        return self.sources, im, im0, dp0[0], None, ''
